...
;;;; main logic
(defun flatten-documentation-list (docs)
  "Take a list of documentation objects and return a flattened list.
The flattening means that for each documentation object the children
are removed and added as toplevel entries to the list.  This happens recursively."
  (loop for doc in docs
     for children = (get-children doc)
     when children append (flatten-documentation-list children)
     when t collect doc
     do (setf (get-children doc) nil)))
...
npackage, as well as for the package itself.  Optionally, you may
supply EXTRA-SYMBOLS that are not external, to be collected as well.
For example, the names of structures or classes may not be exported,
although their accessors are."
  (let* ((*documentation-package* (find-package package))
    ...
    (let ((doc (unless no-package
      ...
      (when doc
        (push doc docs))
      (when flatten
        (setf docs (flatten-documentation-list docs)))
...
If flatten is nil, the default, a generic method with all its defmethods is written
to a single file containing the documentation of them all.

With flatten t, each and individual method documentation is written to its own file.
The names of the method definition files is of the form

    method-<packagename>-<function-name>-NIL-<arg1>-<arg1-spec>-<arg2>-...

If no specializer is specified the corresponding -<argn-spec> is ommitted.
contains invalid Texinfo markup, you lose.

EXTRA-SYMBOLS are for optionally supplying symbols that are not exported
for which we wish to create include files, as well."
